...
-- ============================================
-- COMPLETE CIC TYPE CHECKER
-- Lean 4 / Coq Core Type Theory Alignment
-- ============================================
...
-- ============================================
-- UNIVERSE OPERATIONS (from Universe.x)
-- ============================================
...
indNameFromConstr c ctx = c

applyPatterns :: Term -> [Term] -> [Clause] -> Term
applyPatterns rhs args ps = rhs

findClause :: String -> [Clause] -> Maybe Term
findClause c [] = Nothing
findClause c ((Clause (PCon c' _) rhs):rest) =
...
findReflClause ((Clause (PCon "Refl" _) rhs):_) = Just rhs
...
-- ============================================
-- DEFINITIONAL EQUALITY (from Recursion.x)
-- ============================================
...
-- ============================================
-- UNIVERSE CONSTRAINTS (from Universe.x)
-- ============================================
...
inferLevel ctx t = case whnf ctx t of
  ...
  Var n -> case lookupType n ctx of
    ...
    Nothing -> inferLevel ctx =<< inferSort ctx t
  Pi b body -> do
...
infer ctx t = case t of
  ...
  Lam b@(BExplicit n _) body ->
    Left $ "Cannot infer type of explicit lambda without annotation"
  
  Lam b@(BImplicit n _) body ->
  ...
  Sort SProp -> Right (Sort (SType LZero))
  Sort (SType u) -> Right (Sort (SType (levelSucc u)))
...
checkPatArgs ctx (p:ps) (b:bs) = do
  ...
  Right (bind1 ++ bindRest, Var (Raw "_") : ts')
...
findConstr ind c = find (\cd -> conName cd == c) (indConstrs ind)
...
lookupIndDef tn ctx = Nothing

-- ============================================
-- INDUCTIVE TYPE UNIVERSE CHECKING (Gondor Rules)
-- ============================================
...
checkConstrSort ctx paramLevels indexLevels con = do
  ...
  let minLevel = foldr levelMax LZero (paramLevels ++ indexLevels)
  if levelLeq conLevel minLevel
     then Right ()
     else Left $ "Constructor universe too large: " ++ ppLevel conLevel ++ " > " ++ ppLevel minLevel
...
checkDecl ctx decl = case decl of
  ...
  FunDef n clauses -> do
    ...
    checkRecFunction ctx n ty clauses
    Right ctx
  
  IndDecl ind -> do
    checkInductive ctx ind
    resultSort <- inferIndSort ctx ind
    Right ((Global (indName ind), (Sort resultSort, Nothing)) : ctx)
