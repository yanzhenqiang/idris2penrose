...
void runTests() {
  ...
  // isPre '@' "Y" "N"
  testCaseMore(parenthetically, "`K```@\"#@``:#YK``:#NK;", "", "Y");
  // isPre '#' "Y" "N"
  testCaseMore(parenthetically, "`K```@\"##``:#YK``:#NK;", "", "Y");
  // isPre ')' "Y" "N"
  testCaseMore(parenthetically, "`K```@\"#)``:#YK``:#NK;", "", "N");
  // closes ')' "Y" "N"
  testCaseMore(parenthetically, "`K```@##)``:#YK``:#NK;", "", "Y");
  // closes '@' "Y" "N"
  testCaseMore(parenthetically, "`K```@##@``:#YK``:#NK;", "", "N");
...
void singout() {
  ...
  lvlup(exponentially);
  lvlup(practically);
  FILE *fp = fopen("singularity.boot", "r");
  if (fp) {
    fclose(fp);
...
void rpg() {
  ...
  lvlup_file("parity.x");
  lvlup_file("fixity.x");
  lvlup_file("typically.x");
  lvlup_file("classy.x");
  lvlup_file("barely.x");
  lvlup_file("barely.x");
  lvlup_file_raw("barely.x");
}
